...
import { PROMPT_PRESETS } from "@/lib/ai/constants";
import { PresetSelectionResult, PromptPreset } from "@/lib/ai/types";
...
export async function selectBestPreset(
  ...
  try {
    ...
    const presetDescriptions = PROMPT_PRESETS.map((preset: PromptPreset) => 
...
Set presetId to null only if absolutely no preset fits.${contextSummary}`,
    ...
    if (parsed.presetId) {
      const presetExists = PROMPT_PRESETS.some((p: PromptPreset) => p.id === parsed.presetId);
      if (!presetExists) {
...
export function getPresetSelectionMessage(result: PresetSelectionResult): string {
  ...
  const preset = PROMPT_PRESETS.find((p: PromptPreset) => p.id === result.presetId);
  if (!preset) {
    return 'Using default settings';
  }

...
import { generateGeminiResponse, countTokens } from "@/lib/ai/gemini/services";
import { useAutomaticTitleGeneration } from "@/lib/chat/services/title-generator.service";
import { FileMetadata } from "@/lib/ai/gemini/types";
import { useFileContextStore, useFilesStore } from "@/lib/files/stores";
import { prepareFileContext, enhanceMessageWithContext, generateThinkingContentWithContext } from "@/lib/files/services";
import { buildSystemInstruction } from "@/lib/ai/gemini/utils";
...
export function useChat() {
  ...
  const sendMessage = useCallback(async (content: string, attachedFiles: FileMetadata[] = []) => {
    ...
    setIsStreaming, setTokenCount, generateAndUpdateTitle, stopTypingInterval, globalSystemInstruction
  ]);

...
const SparkleButton: React.FC<SparkleButtonProps> = ({ onClick, children, className = "" }) => {
  ...
  useEffect(() => {
    ...
    particles?.forEach(particle => {
      ...
      element.style.setProperty('--x', `${RANDOM(20, 80)}`);
      element.style.setProperty('--y', `${RANDOM(20, 80)}`);
      element.style.setProperty('--duration', `${RANDOM(6, 20)}`);
      element.style.setProperty('--delay', `${RANDOM(1, 10)}`);
      element.style.setProperty('--alpha', `${RANDOM(40, 90) / 100}`);
      element.style.setProperty('--origin-x', `${Math.random() > 0.5 ? RANDOM(300, 800) * -1 : RANDOM(300, 800)}%`);
      element.style.setProperty('--origin-y', `${Math.random() > 0.5 ? RANDOM(300, 800) * -1 : RANDOM(300, 800)}%`);
      element.style.setProperty('--size', `${RANDOM(40, 90) / 100}`);
    });
  }, []);
    ...
    <>
      <style jsx>{`
        ...
        .sparkle-button button {
          ...
          --bg: radial-gradient(
              40% 50% at center 100%,
              hsl(220 calc(var(--active) * 97%) 72% / var(--active)),
              transparent
            ),
            radial-gradient(
              80% 100% at center 120%,
              hsl(220 calc(var(--active) * 97%) 70% / var(--active)),
              transparent
            ),
            hsl(220 calc(var(--active) * 97%) calc((var(--active) * 44%) + 12%));
          background: var(--bg);
          font-size: 1.125rem;
          font-weight: 500;
          border: 0;
          ...
          border-radius: 100px;
          position: relative;
          box-shadow: 0 0 calc(var(--active) * 6em) calc(var(--active) * 3em) hsl(220 97% 61% / 0.75),
            0 0.05em 0 0 hsl(220 calc(var(--active) * 97%) calc((var(--active) * 50%) + 30%)) inset,
            0 -0.05em 0 0 hsl(220 calc(var(--active) * 97%) calc(var(--active) * 60%)) inset;
        ...
        .sparkle-button button:before {
          ...
          inset: -0.25em;
          z-index: -1;
          border: 0.25em solid hsl(220 97% 50% / 0.5);
          border-radius: 100px;
      ...
      <div className={`sparkle-button ${className}`}>
        <button onClick={onClick}>
          ...
          <span className="text">{children || "Launch now"}</span>
          <ArrowRightIcon className="ml-2 h-5 w-5" />
        </button>
        
        <span aria-hidden="true" className="particle-pen" ref={particleContainerRef}>

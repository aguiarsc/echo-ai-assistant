...
export function FileUpload({
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const tempId = previewUrl; // Use previewUrl as a temporary unique ID
    ...
    try {
      ...
      finalFileData.previewUrl = previewUrl; // Keep the local preview URL
  ...
  const removeFile = async (fileIdentifier: string) => {
    ...
    if (!fileToRemove || !fileToRemove.uri) {
      // If the file is still uploading, it won't have a URI. Just remove locally.
  ...
  // Generate appropriate icon or image preview based on file type
    ...
    if (file.mimeType.startsWith('image/') && file.previewUrl) {
      return (
        <div className="relative w-6 h-6 overflow-hidden rounded">
          {/* Real image preview with Image component as fallback */}
          <img 
            src={file.previewUrl} 
            alt="Preview" 
